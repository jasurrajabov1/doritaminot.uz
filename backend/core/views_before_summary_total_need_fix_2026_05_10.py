...
def get_need_parts(need):
    ...
    additional_need = get_need_addition_total(
...
def get_need_addition_count(institution_id, drug_id, year):
...
class NeedRowDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        related_addition_exists = NeedAddition.objects.filter(
            institution=instance.institution,
            drug=instance.drug,
            year=instance.year,
        ).exists()

        if related_addition_exists:
            return Response(
                {
                    "detail": "Бу эҳтиёж қаторига қўшимча эҳтиёж ёзувлари боғланган. Аввал қўшимча эҳтиёжларни ўчиринг."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class NeedAdditionDetailAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        ...
        write_audit_log(
            ...
            target=item,
            target_type="Қўшимча эҳтиёж",
            description="Қўшимча эҳтиёж янгиланди.",
            extra_data={
                "institution": item.institution.name if item.institution else None,
                "institution_inn": item.institution.inn if item.institution else None,
                "drug": item.drug.name if item.drug else None,
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        drug_name = instance.drug.name if instance.drug else None
        year = instance.year
        added_qty = str(instance.added_qty)
        addition_date = str(instance.addition_date)
        ...
        write_audit_log(
            ...
            action="delete",
            target_type="Қўшимча эҳтиёж",
            description="Қўшимча эҳтиёж ўчирилди.",
            extra_data={
                "id": item_id,
                "repr": item_repr,
                "institution": institution_name,
